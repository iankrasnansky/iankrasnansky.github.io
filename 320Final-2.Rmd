...
authors: "Sharmaine Cameron, Ian Krasnansky, Sonya Radichkova"
date: "May 18, 2020"
...
#### Sharmaine Cameron, Ian Krasnansky, Sonya Radichkova

# Introduction
**Guiding Question:** How do external factors (such as internet access, parent’s education level, parents’ relationship status, family size, etc) impact a student’s grades in secondary education?

Receiving a quality education can be a crucial component to living a fruitful and successful life. Individuals who were high achieving students, which suggests they got the most out of their education, have an easier time finding jobs, benefit from developing professional relationships in school, have improved written and oral communication skills, are more independent, and substantially improve the lives of their future children among other advantages. However, not everyone’s educational experience is the same. Educational experiences can vary greatly as a result of uncontrollable life factors. In this Data Science tutorial, we will explore the impact various life factors may or may not have an impact on a students academic performance. 
# Data Preparation

The first step of any data science project is to either collect data or find a dataset to use. Here, we are using a dataset titled "Student Alcohol Consumption", avalable at https://www.kaggle.com/uciml/student-alcohol-consumption/data. This dataset contains survey results and grade statistics from secondary school students. 
 
After downloading "student-mat.csv", we can import the data and check it out with the following code.
...
2. **famsize:** Family size (binary: 'LE3' - less or equal to 3 or 'GT3' - greater than 3)
3. **Pstatus:** Parent's cohabitation status (binary: 'T' - living together or 'A' - living apart)
4. **Medu:** Mother's education (numeric: 0 - none, 1 - primary education (4th grade), 2 - 5th to 9th grade, 3 - secondary
5. **Fedu:** Father's education (numeric: 0 - none, 1 - primary education (4th grade), 2 - 5th to 9th grade, 3 - secondary
6. **traveltime:** Home to school travel time (numeric: 1 - &lt;15 min., 2 - 15 to 30 min., 3 - 30 min. to 1 hour, or 4 - &gt;1 hour)
...
11. **romantic:** With a romantic relationship (binary: yes or no)
12. **G1:** First trimester grade (numeric: from 0 to 20)
13. **G2:** Second trimester grade (numeric: from 0 to 20)
14. **G3:** Third trimester grade (numeric: from 0 to 20)

If you are confused as to what these attributes represent, remember that you can see a full explanation at https://www.kaggle.com/uciml/student-alcohol-consumption/data. Below is the code for removing the attributes we won't be using (or more accurately, selecting the attributes we will be using).
...
This chart compares the average grades of students who do not plan on going into higher education with those who do. Students who express desire to go into higher education perform better than those who do not by a considerable margin. We should probably use this in our prediction. Next is internet.
...
The romantic stat represents whether a student is in a romantic relationship or not. There is yet again a small correlation, with students in romantic relationships doing slightly worse.
...
# Correlation Matrix
After going through the procedures of EDA, we would like to perform another type of analysis to gain a better understanding of how each attribute is related to other attributes and how each attribute is related to averagegrades. To do this we will use a Correlation Matrix. A Correlation Matrix displays the correlation between attributes using coefficients. The coefficients take on values between negative one and positive one. Coefficient values that are closer to positive one represent a perfect positive relationship while coefficient values that are closer to negative one  are representative of a perfect negative relationship. Coefficient values near zero correspond to attributes that are the least correlated.

Our correlation matrix shows that no attribute is particularly correlated with averagegrades. Most of the coefficient values are near zero and none of them surpass 0.5 or go below -0.5, but there are a few that come close. The attributes with the most negative correspondence to averagegrade according to the correlation matrix are age, traveltime, and romantic. This allows us to assume that a student who is older or lives farther from school or is in a romantic relationship are slightly more likely to have lower grades than if the student is younger or lived closer to school or is not in a romantic relationship. The attributes with the most positive correspondence to averagegrade according to the correlation matrix are Medu, higher, and Fedu. This suggests that a student whose parents reached higher levels of education or a student who intends to reach higher levels of education is slightly more likely to have higher grades than others. Interestingly, the correlation matrix also shows mother’s education is highly positively correlated with father’s education suggesting that if a student’s mother reached higher levels of education it is very likely their father also did too and vice versa. For more information about correlation matrices and their uses, visit https://www.displayr.com/what-is-a-correlation-matrix/. 

```{r}
numeric.var = sapply(mydata, as.numeric)
...
# Random Forest 
For the predictions, we're using a commonly known method as the Random Forest Method. It takes the average from multiple decision trees (which vary in depth) after resampling the training data at random. Because of the randomness of these trees and how resampling takes place multiple times, this helps ensure more accurate predictions. For more information about random forest classification, visit https://www.stat.berkeley.edu/~breiman/RandomForests/cc_home.htm.
...
# Conclusion
Throughout this tutorial, we have shown the use of various data analysis techniques to understand the factors that most impact student academic performance. After performing EDA we hypothesized that the most significant factors were age, parental education, travel time, and time spent studying. From the results of our analyses, we see that each of these factors are potentially good predictors of average grades and our hypothesis was correct.

By determining the factors that can affect student grades, schools can use that information in order to develop methods and resources that can help alleviate the effect of negative factors such as longer travel time and increase student academic performance.

#### References:
**Student Alcohol Consumption Data Set:** https://www.kaggle.com/uciml/student-alcohol-consumption/data
...
**Importance Feature of Random Forests:** https://towardsdatascience.com/explaining-feature-importance-by-example-of-a-random-forest-d9166011959e
